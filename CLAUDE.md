# CLAUDE.md — ServiceNow Architecture Engine v2.8.0 (Tier 2 / Claude Code)
...
**Engine version:** v2.8.1 — authoritative version-of-record for this file. All other references to the engine version across the repo defer to this line.
...
│   └── preflight-traps.md          ← always-on checklist of platform traps; read before any script, patch, flow guide or promotion
...
## Specialist roster (28 specialist personas, 9 with sub-agents)

The full taxonomy and trigger-keyword maps live in `taxonomy.md`. Read that file at routing time when ambiguity arises.

> **Roster count (authoritative, v2.8.1).** 28 distinct specialist personas, each backed by a `SKILL.md` (per the "every specialist has a SKILL.md" rule below) — 9 of them also have a sub-agent under `agents/`. There are **29** `SKILL.md` files under `skills/`: the 28 personas plus `now-assist-genai`, the reference-knowledge companion paired with the Now Assist Specialist builder (not itself a roster persona). This supersedes the earlier "22 / 23 / 25" tallies, which lagged as skills were added. `taxonomy.md` is reconciled to 28 (roster rows 1–28; the FSO Insurance Specialist gateway is row 28, added v2.8.1); the historical version footers below retain their point-in-time tallies as a changelog.
...
- `skills/fso-insurance-specialist/SKILL.md` — FSO Insurance Specialist. Mandatory gateway for ServiceNow Financial Services Operations on the insurance side: FSO Core case/task/policy data model (`sn_bom_*`), Personal/Commercial Lines Servicing (policy operations, change coverage), Insurance Claims (Insurance Claims Core, generic Insurance claims, Personal/Commercial Lines Claims, Individual Life Claims, SIU, reserves/payments approval engine), Underwriting task layer, Individual/Group Life Servicing, Document Processor / Document Intelligence, Complaint Management, Customer Lifecycle / KYC, FSO integrations (remote tables, import sets, Table API, Guidewire, FRISS, Socure, FSM, Service Exchange), Now Assist for FSO, FSO workspace/dashboards and the FSO role model. Fires at Phase 1 Step 5 and Phase 2 Step 4. **Boundary with CSM:** every FSO case sits on the CSM case/customer model — CSM co-fires whenever the base layer (account/contact/consumer, case states, workspace, portals, entitlements) is touched; FSO owns the FSO layer. Treats the FSO-documented table-extension points (new case type, policy type, incident table) as §1.1 halts that need explicit approval. FSO banking packs are catalogued only (out of scope for an insurer). Grounded in `ServiceNowDocs/markdown/financial-services-operations/` (added v2.8.1).
...
- **ITSM Specialist, CSM Specialist, HRSD Specialist, ITOM/Discovery Specialist, CMDB & CSDM Specialist, FSO Insurance Specialist** — mandatory upstream gateways (v2.0 pattern; FSO Insurance v1.0 added in engine v2.8.1) (see Phase 2.1 skills registry above). Fire at Phase 1 Step 5 before any builder dispatch and again at Phase 2 Step 4 in review mode after builder artefacts return.
...
5. **Apply the Domain Expert gateway.** Before routing to any builder specialist — **or before finalizing a domain-scoped *document* deliverable (implementation proposal, scoping document, HLD/LLD/PDD) that makes baseline-process, data-model, or §1.1 claims about a gateway domain** — check whether the task falls within a domain covered by a v2.0 Domain Expert gateway:
   ...
   | Financial Services Operations / FSO, insurance policy servicing, change coverage, claims / FNOL / adjuster / reserve / payment / SIU, underwriting task, life servicing (beneficiary, increase/decrease coverage, term-to-perm, group member), `sn_bom_*` / `sn_ins_*` / `sn_doc_processor_*` / `sn_bom_compl` / `sn_bom_clo_*` / `sn_bom_kyc`, Document Processor, Complaint Management, Customer Lifecycle / KYC, remote tables / Guidewire / FRISS / Socure, Now Assist for FSO | **FSO Insurance Specialist** | `skills/fso-insurance-specialist/SKILL.md` |
...
| ADR (decision record) | One decision per file; immutable once Accepted (supersede, don't edit); records every §1.1 ruling. Template `reference/templates/adr-template.md`; rule `governance-rules.md` §4.1; lives in `clients/<name>/decisions/` |
| Traceability matrix (RTM) | Requirement → story → design → build → test → deploy; append-as-you-go; gap report before sign-off. Template `reference/templates/traceability-matrix-template.md`; rule `governance-rules.md` §4.2; lives in `clients/<name>/traceability.md` |
| RAID log / NFR checklist | Every unresolved OPEN QUESTION becomes a RAID item; NFRs captured at design time and handed to the owning consult. Templates `reference/templates/raid-log-template.md`, `reference/templates/nfr-checklist-template.md`; rule `governance-rules.md` §4.3 |
...
3. Sub-agents and skills currently registered:
   ...
   - **FSO Insurance Specialist** (`skills/fso-insurance-specialist/SKILL.md`, skill only — mandatory gateway, fires Phase 1 Step 5 + Phase 2 Step 4; owns the FSO insurance layer, CSM co-fires on the base case/customer layer) — **v1.0 (engine v2.8.1)**
