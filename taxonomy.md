...
| 28 | FSO Insurance Specialist | ✅ (v1.0 gateway) | ❌ |
...
| **FSO Insurance Specialist** vs **CSM Specialist** | FSO Insurance owns the Financial Services Operations layer (`sn_bom_*` / `sn_ins_*` case types, policy/claim/coverage data model, playbooks, Document Processor, FSO roles). CSM owns the base layer the FSO cases extend (account/contact/consumer, `sn_customerservice_case` states, CSM Configurable Workspace, Customer/Consumer Service Portal, entitlements). | Layer. Any FSO object → FSO Insurance fires; if the base layer is also touched, CSM co-fires and the envelopes reconcile. Banking FSO packs are out of scope for an insurer. | "Change-coverage requests on personal auto policies" → FSO Insurance (CSM co-fires for the Consumer Service Portal). "Account hierarchy for broker agencies" → CSM (FSO Insurance consult flag if policies are attached). |
...
| "FSO", "Financial Services Operations", "insurance policy", "policy servicing", "change coverage", "claim", "FNOL", "adjuster", "reserve", "underwriting", "life servicing", "beneficiary", "Document Processor", "complaint management", "KYC", "sn_bom", "sn_ins", "Guidewire", "FRISS" | FSO Insurance Specialist | CSM Specialist (co-fires on the base case/customer layer) |
